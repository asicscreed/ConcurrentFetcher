...
export type myCallback =
           (uniqueId: string,
            ...
            error: Error | null,
            abortManager: AbortManager) => void;
...
export class ConcurrentFetcher {
  ...
  constructor(requests: RequestItem[]) {
    if (!Array.isArray(requests)) { throw this.CommonError("InvalidArgument", "requests"); }
    const reqLen = requests.length;
    if (reqLen < 1) { throw this.CommonError("ArgumentEmpty", "requests"); }
    const reqArray = [];
    for (let i = 0; i < reqLen; i++) { reqArray.push(i); }
  ...
  /**
    ...
    */
  async fetchWithRetry<T>(url: string | Request, fetchWithSignal: RequestInit, uniqueId: string, maxRetries: number, statusCodesToRetry: number[][], retryDelay: number, forceReader: boolean, cutoffAmount: number, progressCallback: myprogressCallback | undefined, countRetries = 0): Promise<T> {
    ...
    try {
      ...
      if (forceReader && !response.bodyUsed && response.body) { forceResponseReader = true; }
      let returnData;
   ...
   *  - errors: { uniqueId: string; url: string | Request; error: Error }[];
   */
  async concurrentFetch({ progressCallback }: ConcurrentFetchOptions = {}): Promise<ConcurrentFetchResponse> {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const results: any[] = [];
    ...
    const fetchPromises = this.requests.map((request, index) => {
      ...
      .finally(() => {
          completedCount++;
          if (progressCallback) { progressCallback(uniqueId, completedCount, this.requests.length, 0, 0); }
      });
    });
   ...
   *
   */
  async fetchTextStream(fetchResponse: Response, fetchType: string, uniqueId: string, contentType: string, contentLength: number, progressCallback: myprogressCallback | undefined): Promise<string> {
      ...
      while ({ done, value } = await reader.read(), !done) {
          ...
          if (progressCallback) { progressCallback(uniqueId, 0, 0, textChunks.length, contentLength); }
      }
      // empty buffer...
   ...
   *
   */
  async fetchBlobStream(fetchResponse: Response, uniqueId: string, contentType: string, contentLength: number, progressCallback: myprogressCallback | undefined): Promise<Blob> {
      if (!fetchResponse.body) { throw new Error('Response body is empty.'); }
  ...
  shouldRetryRequest(responseStatus: number, maxRetries: number, statusCodesToRetry: number[][], countRetries: number): boolean {
    if ((maxRetries-countRetries) < 1) { return false; }
    let isInRange = false;
